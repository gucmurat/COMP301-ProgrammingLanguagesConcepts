  ...
  (define value-of
    (lambda (exp env)
      (cases expression exp
        ...
        ;counter is stored in the pair consisting of arg and count.
        ;then pair is sent to apply-procedure.
    ...
    (lambda (proc1 arg)
      (cases proc proc1
        ...
        (nested-procedure (name var body saved-env count)
                   (begin
                    ; recursive-displayer procedure takes number from argument containing count as its second element
                    (recursive-displayer  name (expval->num (cdr arg)))
                    ; both count and the variable are added to the environment by using extend-env procedure.
