(module tests mzscheme
  ...
  (define test-list
    '(
      ...
      (additional-test-1 "let m=2 in letrec divide-by-two(x) = if zero?(x)
                             then 0 else -((divide-by-two -(x, m)),-1) in (divide-by-two 14)" 7)

      (additional-test-2 "letrec mod2?(x) = if zero?(x) then 0 else if zero?(-(x,1)) then 1
                             else (mod2? -(x, 2)) in (mod2? 7)" 1)

      (additional-test-3 "let a = 5 in let a? = proc (x) if zero?(-(x,a)) then 1 else 0 in (a? 5)" 1)
