  ...
  (define translation-of
    (lambda (exp env)
      (cases expression exp
        ...
        ; #####################################################
        ;Parameters are determined according to description.
        (proc-exp (var body)
          (proc-nested-exp var 'n 'anonym (translation-of body env))
        )
        ; #####################################################
        ...
        (call-exp (rator rand)
           (cases expression rator
             ; The case where the operator is a var-exp
              (var-exp (var)
                   (call-nested-exp
                    (translation-of rator env)
                    (translation-of rand env)
                    ;;Increment the count as the parameter of call-nested-exp by using diff-exp 
                    (diff-exp (var-exp 'n) (const-exp -1))))
             ; The case where it is not in the environment. Hence, it is initialized as 1
             (else         
              (call-nested-exp
               (translation-of rator env)
               (translation-of rand env)
        ...
        (letrec-exp (p-name b-var p-body letrec-body)
         (letrec-nested-exp p-name b-var 'n (translation-of p-body env) (translation-of letrec-body env))
        )
        ; #####################################################
